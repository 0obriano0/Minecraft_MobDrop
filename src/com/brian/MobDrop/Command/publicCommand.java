		...
		try {
			...
			for (MobDropItems MobDropItems : lstMobDropItems)
			{
				if(MobDropItems.Quantity == MobDropItems.Quantity_max) {
					sender.sendMessage("§f  - " + MobDropItems.ItemName + " §a(§f" + MobDropItems.Chance + "%§a掉落§f " + MobDropItems.Quantity + " §a個)");
				}else {
					sender.sendMessage("§f  - " + MobDropItems.ItemName + " §a(§f" + MobDropItems.Chance + "%§a掉落§f " + MobDropItems.Quantity + "-" + MobDropItems.Quantity_max + " §a個)");
				}
			}
		}
		catch (NullPointerException e)
		{
			sender.sendMessage("§c查無「§n " + DataBase.GetEntityName(keyName) + " §r§c」生物，請重新查詢");
		}
		return true;
	}
